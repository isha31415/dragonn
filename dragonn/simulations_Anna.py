...
def motif_density(motif_name, seq_length, num_seq,
    ...
    myModel=SequenceDNN(1000);
    for x in range(0,len(sequence__and_embedding_arr)):
        for y in range(0, len(sequence__and_embedding_arr[x])):
            print(embedding_arr[x][y].what)
            print(embedding_arr[x][y].startPos)
    myModel.deeplift(sequence__and_embedding_arr, batch_size=200)
    pdb.set_trace() 
    return sequence_arr, embedding_arr
    #return sequence_arr, embeddings_for_each_seq
...
def pos_neg(motif_name, seq_length, num_seq,
    ...
    positive_set = [motif_density(motif_name, seq_length, num_seq,
                      min_counts, max_counts, GC_fraction,
                      central_bp=None)]

    random.shuffle(positive_set)
    validation_positive_set = positive_set[:(0.3 * (len(positive_set)))]
    training_positive_set = positive_set[(0.7 * (len(positive_set))):]

    negative_set = [motif_density(motif_name, seq_length, num_seq,
                      min_counts = 0, max_counts = 0, GC_fraction = .4,
                      central_bp=None)]

    validation_negative_set = negative_set[:(0.3 * (len(negative_set)))]
    training_negative_set = negative_set[(0.7 * (len(negative_set))):]
    ...
    negative_labels = np.ones(training_negative_set.shape)
...
motif_density("TAL1_known4", 1000, 50, 2, 4, .4, central_bp=None)
